...
<div class="max-w-6xl mx-auto mt-8 px-4 bg-gray-300 p-6 rounded-xl shadow-md">
    ...
    <div class="flex flex-col md:flex-row md:items-center md:justify-between mb-6 gap-4">
        ...
        <a href="{{ route('producto.add') }}"
            class="cursor-pointer bg-amber-400 hover:bg-amber-500 text-gray-900 px-6 py-2.5 font-semibold rounded-lg shadow transition text-center whitespace-nowrap">
            Agregar Producto
        </a>
    </div>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
        <div class="bg-white p-4 rounded-lg shadow border-l-4 border-blue-500">
            ...
            <p class="text-xl font-bold">{{ $productos->count() }}</p>
        </div>
        <div class="bg-white p-4 rounded-lg shadow border-l-4 border-red-500">
            <p class="text-gray-500 text-sm">Stock Mínimo</p>
            <p class="text-xl font-bold">{{ $productos->where('stock', '<=', 'stock_minimo')->count() }}</p>
        </div>
        <div class="bg-white p-4 rounded-lg shadow border-l-4 border-green-500">
            <p class="text-gray-500 text-sm">Servicios</p>
            <p class="text-xl font-bold">{{ $productos->where('tipo', 'servicio')->count() }}</p>
        </div>
        <div class="bg-white p-4 rounded-lg shadow border-l-4 border-purple-500">
            <p class="text-gray-500 text-sm">Productos</p>
            <p class="text-xl font-bold">{{ $productos->where('tipo', 'producto')->count() }}</p>
        </div>
    </div>
                ...
                @foreach ($productos as $producto)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td @class([
                            ...
                            'text-red-600 font-bold' => $producto->stock_minimo >= $producto->stock && $producto->tipo == 'producto',
                        ])>
