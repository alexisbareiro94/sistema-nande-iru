...
async function getCaja(id) {
    try {
        const res = await fetch(`http://localhost:8080/api/caja/${id}`, {
            method: 'GET',
            headers: {
...
function mapearDetalleCaja(data) {
    ...
    if (data.datos.caja.monto_cierre > data.datos.caja.saldo_esperado) {
        divOne.classList = 'bg-gradient-to-br from-green-50 to-green-100 border border-green-200 rounded-xl p-4'
        divDos.classList = 'p-2 bg-green-500 rounded-lg'
        dcDiferencia.classList = 'text-xl font-bold text-green-600'
        simbolo = "+";
    } else {
        divOne.classList = 'bg-gradient-to-br from-red-50 to-red-100 border border-red-200 rounded-xl p-4'
    ...
    fechaApertura = fechaAp.toLocaleString('es-PY', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric',
        hour: '2-digit',
    ...
    fechaCierre = fechaCie.toLocaleString('es-PY', {
        day: '2-digit',
        month: '2-digit',
        year: 'numeric',
        hour: '2-digit',
    ...
    const efecPorcentaje = data.datos.efecPorcentaje ?? data.datos.efecProcentaje ?? 0;
    const transfPorcentaje = data.datos.transfPorcentaje ?? data.datos.transfProcentaje ?? 0;
